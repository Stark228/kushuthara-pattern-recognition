...
import {View, StyleSheet,  Dimensions, Text, ActivityIndicator, Pressable, TouchableOpacity, Image,
} from 'react-native';
...
// const modelWeights = require(myBinaryFile);
  ...
  const processImagePrediction = async (base64Image) => {
    ...
    try {
      ...
      if (prediction && prediction.length > 0) {
        const highestPrediction = tf.argMax(prediction).dataSync()[0];
        const result = RESULT_MAPPING[highestPrediction];
        // console.log('Current Animal:', RESULT_MAPPING[highestPrediction]);
        setIsProcessing(false);
        navigation.navigate('ImageDisplay', { result, imageUri: base64Image.uri });
      }
    } catch (error) {
      console.error(error);
    }
